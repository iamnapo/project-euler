module.exports = () => {
	...
	for (let i = 0; i < radicals.length; i += 1) radicals[i] = new Radical(1, i);

	for (let i = 2; i <= limit; i += 1) {
		if (radicals[i].rad === 1) {
			radicals[i].rad = i;
			for (let j = i + i; j < limit; j += i) radicals[j].rad *= i;
		}
	}
	...
	for (let c = 3; c <= limit; c += 1) {
		...
		for (const a of sortedRadicals) {
			...
			if (a.number < chalf) {
				const b = c - a.number;
				if (a.rad * radicals[b].rad * radc < c && gcd(a.rad, radicals[b].rad) === 1) result += c;
			}
		}
	}
