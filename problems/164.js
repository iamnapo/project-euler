module.exports = () => {
	...
	return console.log(`Problem 164 solution is: ${x.flat().reduce((a, b) => a + b, 0)}`);
};

function initCounts(digitSum) {
	const x = new Array(10).fill([]);
	for (let i = 0; i < 10; i += 1) x[i] = new Array(10).fill(0);
...
function nextCounts(x, digitSum) {
	const y = new Array(10).fill([]);
	for (let i = 0; i < 10; i += 1) y[i] = new Array(10).fill(0);
